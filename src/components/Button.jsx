...
const buttonClassNames = {
    default: "text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 ",
    alternative:
...
const Button = ({
    ...
    return (
        <div
            className={`${
                disabled || loading ? "opacity-50 pointer-events-none" : "opacity-100 pointer-events-auto"
            } ${className}`}
        >
            <button
                type={type}
                className={`flex items-center font-medium rounded-lg text-sm px-5 py-2.5 text-center whitespace-nowrap border border-transparent ${buttonClassNames[variant]}`}
                onClick={onClick}
            >
                {loading && (
                    <svg
